...
def TEST_AREA(gbs_url):
    ...
    for j in range(0, 10000):
        ...
        allResult = False
        if result_channel1 and result_channel2:
            allResult = True
        else:
            allResult = False
        seleniumObject.toTxt("{} 第{}次通道逻辑测试结果为：{}\n, 对应测试结果分别为：\n"
                             "通道1拉流结果：{}\n"
                             "通道2拉流结果：{}\n"
                             .format(_cur_ip, j, allResult, result_channel1, result_channel2))
        if not allResult:
            seleniumObject.toTxt(f"{_cur_ip} 停止测试")
            break
        else:
            seleniumObject.toTxt(f"{_cur_ip}继续测试")
            waitTimeBack(3)
